	...
	public function beverage_table_shortcode() {
		...
		if ( $the_query->have_posts() ) {
			...
			while ( $the_query->have_posts() ) :
				...
				$output .= '<tr>' . "\n";
				$output .= '<td nowrap align="center"><a href="#TB_inline?&width=600&height=550&inlineId=beverage-' . get_the_ID() . '" class="thickbox"><span class="dashicons dashicons-info-outline"></span> <span class="screen-reader-text">Info</span></a></td>' . "\n";
				$output .= '<td>' . get_the_title() . '</td>' . "\n";
				$output .= '<td>' . esc_html( $vendors_array[0]->name ) . '</td>' . "\n";
				...
				$output .= get_the_post_thumbnail( get_the_id(), array( 200, 500 ), array( 'class' => 'alignright' ) );
